...
Keep one clear theme source such as `assets/css/theme.css`:
...
A single brand seed should derive primary hover, active, soft, border, and focus values. Do not scatter raw brand values across HTML pages.
...
- `direct`: semantic markup and Tailwind transfer with minor JSX syntax edits
